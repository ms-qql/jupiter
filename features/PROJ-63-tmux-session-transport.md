...
## BUG-4 (Production-Vorfall, 2026-07-07): `POST /sessions` hängt für IMMER bei `transport=tmux`

**Gemeldet von:** Nutzer, direkt nach Aktivierung von `engine_overrides: {codex: tmux, opencode: tmux}` per `backend/config/transport.yaml`. Erste echte Codex-Session im tmux-Modus ("TMUX Test") — Dialog blieb für immer bei „Startet…" hängen (`new-session-dialog.tsx`: `submitting` wird nur im `finally` zurückgesetzt, das `await createSession()` aber nie zurückkehrte).

### Root Cause (verifiziert)
- **Kein** Bug im Codex-/OpenCode-Turn selbst: `out.log`/`err.log` der betroffenen tmux-Session zeigten einen vollständig korrekten Turn (`turn.completed`, Exit-Code 0) — die Engine lief im Hintergrund einwandfrei durch.
- Der komplette heutige `jupiter-backend`-Access-Log enthielt **keine einzige** abgeschlossene Zeile für dieses `POST /sessions` — der Request bekam nie eine Antwort, nie einen Fehler, nie ein Timeout.
- `ps -ef` zeigte den fuer `TmuxTransport.spawn()` erzeugten `tmux start-server ; … ; new-session -d …`-Subprozess **noch Minuten später aktiv** (PID 765861, `PPid=1`, sauber zum tmux-Server daemonisiert, `fd 0/1/2 → /dev/null`) — das beweist: `await proc.communicate()` in `TmuxTransport._tmux()` ist fuer diesen einen Aufruf nie zurückgekehrt, obwohl der eigentliche tmux-Vorgang laengst durchgelaufen war.
- **Sechs** Live-Reproduktionsversuche gegen den echten Produktions-Prozess (sequenziell + 4× parallel, mit einem selbst ausgestellten JWT) liefen alle **fehlerfrei und schnell** (0.05–0.18 s) durch — der Hang ist selten/nicht deterministisch, kein einfacher Timing-Bug in der Spawn-Logik selbst.
- **Unabhängiger, bestätigender Beweis für dieselbe Fehlerklasse:** im laufenden `jupiter-backend`-Prozess fand sich ein **echter, dauerhaft nicht eingesammelter Zombie-Prozess** (`[systemctl] <defunct>`, direktes Kind von `jupiter-backend`) — erzeugt von `metrics.py::_systemctl_is_active()` (Healthcheck fuer `/metrics/status`), das `asyncio.create_subprocess_exec(...).communicate()` ebenfalls ohne zuverlässige Reaping-Garantie nutzt. Das beweist: **asyncio-Subprozess-Reaping ist in diesem Prozess unter realer Last nicht 100% zuverlässig** — ein seltenes, aber reales Verhalten dieser Laufzeitumgebung, keine Erfindung.
- `TmuxTransport._tmux()` hatte — anders als `metrics.py`s Healthcheck (der wenigstens ein 5s-Timeout hat) — **gar keine Zeitbegrenzung**. Trifft die seltene Reaping-Störung ausgerechnet einen der `_tmux()`-Aufrufe innerhalb von `spawn()` (has-session/kill-session/chained new-session/pane_pid), hängt `driver.start()` und damit der gesamte `POST /sessions`-Request **für immer**, ohne Wiederherstellungsmöglichkeit außer Backend-Neustart.

### Fix
- `backend/app/config.py`: neue Einstellung `tmux_cmd_timeout_seconds` (Default 10s), analog zu `metrics_systemctl_timeout_seconds`.
- `backend/app/engine/transport.py`: `TmuxTransport._tmux()` umschließt `communicate()` jetzt mit `asyncio.wait_for(..., timeout=tmux_cmd_timeout_seconds)`. Bei Timeout: `proc.kill()` + best-effort `proc.wait()` (Zombie-Vermeidung), danach ein klarer `TransportError("tmux … antwortet nicht (Timeout nach …s)")` — der bereits bestehende `except TransportError` in `routes/sessions.py` (BUG-2-Fix) macht daraus eine saubere `503`-Antwort statt eines nie endenden Requests. Betrifft transitiv ALLE `_tmux()`-Aufrufer (`spawn()`, `_probe_alive()`/Liveness, `terminate()`/`kill()`).
- Neuer Regressionstest `test_hanging_tmux_call_times_out_instead_of_hanging_forever` (`backend/tests/test_proj63_tmux_transport.py`): Fake-`tmux`-Binary, das nie antwortet → `spawn()` muss innerhalb des konfigurierten Timeouts einen `TransportError` werfen statt zu hängen. Die zugrunde liegende, seltene Reaping-Störung selbst ist nicht deterministisch reproduzierbar — der Test deckt die Absicherung ab, nicht die Ursache.
- **Regression:** vollständiger Backend-Testlauf `pytest` → 1123 passed, 1 unabhängiger, vorbestehender Fehlschlag (`test_proj50_codex_abc.py::test_generator_check_passes_no_drift`, bereits vor PROJ-63 bekannt), 1 neuer Test grün.
- **Nicht behoben (bewusst, außerhalb dieses Bugs):** die zugrunde liegende, seltene asyncio-Reaping-Störung selbst (Ursache im Laufzeit-/OS-Verhalten, nicht in Jupiter-Code — kein `os.waitpid`/Signal-Handler-Override im Repo gefunden). Der Fix macht sie **sichtbar und bounded** (klarer 503 statt stillem Hang) statt sie zu beseitigen. Der `metrics.py`-Zombie selbst (Healthcheck) wurde nicht angefasst — separates, geringeres Risiko (nur ein Status-Feld, kein Nutzer-Request hängt daran); ggf. eigenes Ticket, falls Zombies sich häufen.
